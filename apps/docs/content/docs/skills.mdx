...
- **Off — Ignore (default)**: the runtime is launched against an isolated config directory; only workspace skills are available. Recommended for any agent shared with teammates.
- **On — Merge**: the runtime inherits the user-global skill directory in addition to workspace skills. Use this for personal agents that intentionally rely on locally installed skills.

Workspace skills under `{workDir}/.claude/skills/` are loaded regardless of the toggle — this switch only governs the host's user-global directory. Codex already isolates skill discovery via `CODEX_HOME` and ignores the toggle.
