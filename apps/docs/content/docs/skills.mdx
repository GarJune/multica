...
<Callout type="info">
Multica maintains an official [`multica-cli`](https://github.com/multica-ai/multica-cli) skill for Claude Code, Codex, Cursor, and other local agents. It teaches agents how to use the authenticated `multica` CLI safely, including issue comments, metadata, pull requests, mentions, and status side effects. Import it from GitHub with `https://github.com/multica-ai/multica-cli/tree/main/skills/multica-cli`, or install it directly into your local coding tool from the repository.
</Callout>
