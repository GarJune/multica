...
- **关闭 —— Ignore（默认）**：运行时使用隔离的配置目录，只能看到工作区 Skill。**推荐用于团队共享智能体**。
- **开启 —— Merge**：运行时同时继承本机的用户级 skill 目录和工作区 Skill。适合个人智能体、且明确依赖本机 skill 的场景。

无论开关如何，工作区目录下的 `{workDir}/.claude/skills/` 都会照常加载——这个开关只控制本机用户级目录。Codex 本身已经通过 `CODEX_HOME` 做了隔离，所以开关对 Codex 没有影响。
